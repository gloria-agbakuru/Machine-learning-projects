...
class ReLU(Layer):
    ...
    def backward(self, grad_in: np.ndarray) -> Layer.BackwardResult:
        ...
        d_x = np.where(relu_x > 0, grad_in, 0.0)
...
class Linear(Layer):
    ...
    def backward(self, grad_in: np.ndarray) -> Layer.BackwardResult:
        ...
        dW = np.dot((x).transpose(),grad_in)
        db = np.sum(grad_in, axis=0)

        d_inputs = np.dot(grad_in, W.transpose())
...
class SoftmaxCrossEntropy(Loss):
    def forward(self, inputs: np.ndarray, targets: np.ndarray) -> float:
        ...
        exp_sum = np.sum(np.exp(y), axis=1, keepdims=True)
        softmax = np.exp(y)/exp_sum
...
def verify(images: np.ndarray, targets: np.ndarray) -> Tuple[int, int]:
    ...
    a = np.argmax(model.forward(images),axis=1)

    num_ok = np.sum(a==targets)
    total_num = len(a)
...
for epoch in range(200):
    ...
    current_loss_value = loss_value / len(train_indices)
    ...
    if validation_accuracy > best_validation_accuracy:
        ...
        best_epoch = epoch
        print("\tEpoch %d currently has the best accuracy equal to %.2f%%." % (
            best_epoch, best_validation_accuracy))
    else:
